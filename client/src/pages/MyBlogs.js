...
import { useFetchData } from '../hooks/useFetchBlog.js'
...
const MyBlogs = () => {
    ...
    const { fetchBlogs } = useFetchData()
    const loadBlogs = async () => {
        await fetchBlogs()
    }
    useEffect(() => {
        console.log('loading user blog...')
        loadBlogs()
    }, [])
    return (
        <section className="py-6 sm:py-12 bg-gray-100 text-gray-800">
            <div className="container p-6 mx-auto space-y-8">
                ...
                <div className="grid grid-cols-1 gap-x-4 gap-y-8 md:grid-cols-2 lg:grid-cols-4">
                    {
                        isLoading ? <p>Data fetching</p> : blogs && blogs.length > 0 ?
                            blogs.map(blog => (
